 ...
 */
const initAuth = (context) => __awaiter(void 0, void 0, void 0, function* () {
    console.log('Iniciando autenticación');
    const config = {};
    // obtener clave OpenAI
    let openaiKey = yield context.secrets.get(OPENAI_API_KEY);
    if (process.env["OPENAI_TOKEN"]) {
        openaiKey = process.env["OPENAI_TOKEN"];
    }
    // obtener clave Anthropic
    let anthropicKey = yield context.secrets.get(ANTHROPIC_API_KEY);
    ...
    if (!openaiKey) {
        ...
        openaiKey = yield setNewOpenAIKey(context);
    }
    // verificar y configurar Anthropic
    if (!anthropicKey) {
        console.log("Clave API de Anthropic no encontrada");
        anthropicKey = yield setNewAnthropicKey(context);
    }
 ...
 */
const setNewOpenAIKey = (context) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const secret = yield vscode.window.showInputBox(inputBoxOptions);
    if (!secret) {
        vscode.window.showWarningMessage('No se ha recibido ninguna clave API.');
        return "";
    }
    yield context.secrets.store(OPENAI_API_KEY, secret);
    return secret;
});
/**
 ...
 * @returns {Promise<string>} Clave API
 */
const setNewAnthropicKey = (context) => __awaiter(void 0, void 0, void 0, function* () {
    const inputBoxOptions = {
    ...
    yield context.secrets.store(ANTHROPIC_API_KEY, secret);
    return secret;
});
/**
