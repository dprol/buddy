...
class OpenAIProvider extends AIProvider {
    ...
    processResponse(response) {
        ...
        if (response.data.usage?.total_tokens >= this.getModelLimits().maxTokens) {
            vscode.window.showWarningMessage(
                `La respuesta utilizó ${response.data.usage.total_tokens} tokens, ` +
    ...
    handleError(error) {
        if (error?.message === "canceled") {
            ...
            vscode.window.showErrorMessage(
                `Error al comunicarse con OpenAI: ${error.message}`
            );
        }
        throw error;
    }
